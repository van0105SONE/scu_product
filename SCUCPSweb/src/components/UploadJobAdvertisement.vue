  ...
  <div class="flex flex-col items-center justify-center">
    ...
    <div class="container bg-gray-200 shadow-xl m-10 rounded-lg">
      <div class="flex flex-col items-center justify-center w-full mt-5">
        <img
          :src="jobForm.Image == '' ? '/src/assets/icons/picture.png' : jobForm.Image "
...
export default {
  ...
  methods: {
    ...
    UploadJobAdvertise() {
      debugger;
      this.jobForm.Phone = this.jobForm.Phone.toString();
      if (this.jobForm) {
        axiosClient.post("Job/UploadJob", this.jobForm).then((response) => {
          if (response.status == 200) {
             router.push({name: 'jobmanagement'})
          } else {
            notifierErrorq("ບາງຢ່າງຜິດພາດບໍ່ສາມາດລຶບຂໍ້ມູນໄດ້");
          }
        });
      } else {
        notifierError("ກາລຸນາປ້ອນຂໍ້ມູນກ່ອນ");
      }
    },
    ChooseImage() {
